...
class Test_Struct_Validatable_SpecificCondition < Test::Unit::TestCase
  ...
  members = [
    :list_only_int, :true_or_false, :like_str, :has_foo, :has_foo_and_bar,
    :one_of_member, :has_ignore, :nand, :all_pass, :catch_error, :no_exception, :not_integer
  ]

  Sth = Struct.new(*members) do
    validator :list_only_int, GENERICS(Integer)
    validator :true_or_false, BOOL?
    validator :like_str, STRINGABLE?
    validator :has_foo, CAN(:foo)
    validator :has_foo_and_bar, CAN(:foo, :bar)
    validator :one_of_member, MEMBER_OF([1, 3])
    validator :has_ignore, AND(1..5, 3..10)
    validator :nand, NAND(1..5, 3..10)
    validator :all_pass, OR(1..5, 3..10)
    validator :catch_error, CATCH(NoMethodError){|v|v.no_name!}
